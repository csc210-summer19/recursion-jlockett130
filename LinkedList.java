...
public class LinkedList<E extends Comparable<E>> {
	...
	private E get(Node ref, int startIndex, int stopIndex) {
		if (startIndex == stopIndex) {
			...
			startIndex++;
			return get(ref.next, startIndex++, stopIndex);
		}
	}
	...
	public void removeAll(E el) {
		helpRemoveAll(first, el);
	}

	private void helpRemoveAll(Node ref, E el) {
		if (ref != null) {
			if (first.data.equals(el)) {
				...
				if (ref.next != null) {
					if (ref.next.data.equals(el)) {
						...
						helpRemoveAll(ref, el);
						n--;
					} else {
						helpRemoveAll(ref.next, el);
					}
				}
			}
		}
	...
	private void helpDupAll(Node ref, E el) {
		if (ref != null) {
			if (ref.data.equals(el)) {
				Node dup = new Node(el, ref.next);
				ref.next = dup;
				n++;
				helpDupAll(dup.next, el);
			}
		}
	}
